...
# a measure of the information content a term holds for a particular document in a document collection.

# However, he just started learning to code and somehow his numbers do not match what is expected.
...
# the number of times a term occurs in a document (THIS WORKS)
...
# The problem lies with lines 30 to 33.
# `number_of_documents` and `number_of_documents_with_term` are both integers.
# When we call the Integer#/ method, the decimal points (or the remainder) are truncated.
# The return value of the Integer#/ method is an integer.
# Thus, the return value of `Math.log(number_of_documents / number_of_documents_with_term)` is an imprecise value.
# To obtain a float value, we must perform float division.
# We convert either # `number_of_documents` and `number_of_documents_with_term` into a float point number by using Integer#to_f.
# Further exploration - if none of the documents contain the search term,
# `number_of_documents_with_term` will be 0.
# Consequently, `number_of_documents / 0` will be Infinity.
